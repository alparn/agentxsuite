...
export function CanvasEdgeSidebar({ edge, onClose, onUpdate, onDelete, sourceNodeType, targetNodeType }: CanvasEdgeSidebarProps) {
  ...
  return (
    <div className="w-96 bg-slate-900 border-l border-slate-800 flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex-1 p-4 space-y-4">
        ...
        <div>
          ...
          {sourceNodeType && targetNodeType ? (
            <select
              ...
              onChange={(e) => {
                ...
                const validTypes = getValidEdgeTypes(sourceNodeType as any, targetNodeType as any);
                if (validTypes.includes(newType)) {
                  onUpdate(edge.id, { ...edgeData, type: newType });
                } else {
                  console.warn(`Invalid edge type ${newType} for ${sourceNodeType} -> ${targetNodeType}`);
                }
              }}
            ...
            >
              {getValidEdgeTypes(sourceNodeType as any, targetNodeType as any).map((type) => (
          ...
          {sourceNodeType && targetNodeType && (
            <p className="text-xs text-slate-500 mt-1">
              Valid types for {sourceNodeType} → {targetNodeType}
            </p>
          )}
        </div>
        ...
        {/* Metadata */}
        {edgeData.metadata && Object.keys(edgeData.metadata).length > 0 && (
          <div>
